...
if(!empty($_GET['pengaturan'])){
	$nama= htmlentities($_POST['namaklinik']);
	...
	$data[] = $kontak;
	$data[] = $id;
	$sql = 'UPDATE klinik SET nama_klinik=?, alamat_klinik=?, tlp=? WHERE id_klinik = ?';
...
if(!empty($_GET['customer'])){
	$id = htmlentities($_POST['id']);
		...
		$data[] = $tgl;
		$data[] = $id;
	$sql = 'UPDATE customer SET nama_customer=?, gender=?, alamat=?, kontak=?, tgl_daftar=?  WHERE id_customer=?';
	$row = $config -> prepare($sql);
	$row -> execute($data);
...
if(!empty($_GET['barang'])){
	...
	$data[] = $tgl;
	$data[] = $id;
	$sql = 'UPDATE barang SET id_kategori=?, nama_barang=?, harga_jual=?, satuan_barang=?, stok=?, tgl_update=?  WHERE id_barang=?';
	$row = $config -> prepare($sql);
	$row -> execute($data);
...
if(!empty($_GET['profil'])){
	...
	$card_id = htmlentities($_POST['card_id']);
	...
	$data[] = $card_id;
	$data[] = $id;
	$sql = 'UPDATE member SET nm_member=?,alamat_member=?,telepon=?,email=?,card_id=? WHERE id_member=?';
	$row = $config -> prepare($sql);
	$row -> execute($data);
	echo '<script>window.location="../../index.php?page=user&success=edit-data"</script>';
}
